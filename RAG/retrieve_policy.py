...
index = vsc.get_index(index_name="newcatalog.gold.policies_embeddings")
...
def retrieve_policies_for_claim(final_explanations):
   # Create a clean, comma-separated list of attributes
    ...
    retrieval_query = f"Policy search for insurance claims with features: {attributes_str}."


    results = index.similarity_search(
        query_text= retrieval_query,
        # query_vector= "Add custom calculated query embedding for similarity search or relevant retrieval"
        columns=["policy_id", "chunk_id", "chunk_text"],
        num_results=3
    )
...
if __name__ == "__main__":
    ...
    policies = retrieve_policies_for_claim(final_explanations)

    print("Relevant policies retrieved for the claim:" \
    "" \
    "" \
    "" \
    "")
